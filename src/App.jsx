...
function minutiFromDb(row) {
  // row.minutes è la colonna jsonb "vera"; per compatibilità con righe create prima
  // della migrazione (colonne fisse lesing/semplificato/tecnico/hobby) ricadiamo su quelle.
  const legacy = { lesing: row.lesing || 0, semplificato: row.semplificato || 0, tecnico: row.tecnico || 0, hobby: row.hobby || 0 };
  const minutes = row.minutes && Object.keys(row.minutes).length > 0 ? row.minutes : legacy;
  return { id: row.id, date: row.date, minutes };
}
function minutiToDb(entry, userId) {
  return { user_id: userId, date: entry.date, minutes: entry.minutes };
}
...
function MiniCalendar({ cursor, setCursor, entries, statusFn, detailFn, onDayClick }) {
      ...
      <div className="cal-grid">
        ...
        {cells.map((d, i) => {
          ...
          return (
            <div
              key={i}
              className={`cal-cell clickable ${inMonth ? "" : "outside"} ${cls} ${ds === today ? "today" : ""}`}
              onClick={() => onDayClick && onDayClick(ds, entry || null)}
            >
              <div className="d">{d.getDate()}</div>
              {tag && <div className="tag">{tag}</div>}
              {entry && detailFn && <div className="detail">{detailFn(entry)}</div>}
            </div>
          );
        })}
      </div>
...
function DayDetailModal({ data, onClose, onGoToForm }) {
  if (!data) return null;
  const { date, kind, entry, settings, habits } = data;

  return (
    <div className="backdrop center" onClick={onClose}>
      <div className="day-modal" onClick={(e) => e.stopPropagation()}>
        <div className="day-modal-head">
          <h4>{fmtDateIt(date)}</h4>
          <button className="icon-btn" onClick={onClose}><Icon.x width={14} height={14} /></button>
        </div>

        {!entry ? (
          <div className="day-modal-empty">
            <p>Nessuna voce registrata per questo giorno.</p>
            <button className="btn primary" onClick={() => onGoToForm(date)}>Registra questa giornata</button>
          </div>
        ) : kind === "minuti" ? (
          <>
            <div className="day-modal-badge">
              <span className={`badge ${p1Status(p1Total(entry, settings), settings)}`}>
                {(() => { const s = p1Status(p1Total(entry, settings), settings); return s[0].toUpperCase() + s.slice(1); })()}
              </span>
              <span className="mono day-modal-total">{p1Total(entry, settings)} min · {fmtHM(p1Total(entry, settings))}</span>
            </div>
            <div className="day-modal-list">
              {activeCategoriesFor(settings, entry.date).map((c) => (
                <div className="day-modal-row" key={c.key}>
                  <span>{c.label}</span>
                  <span className="mono">{entry.minutes[c.key] || 0} min</span>
                </div>
              ))}
            </div>
          </>
          ...
          <>
            <div className="day-modal-badge">
              {(() => { const { spunte, saltate } = p2Counts(entry, habits); const st = p2Status(entry, habits);
                return <>
                  <span style={{ fontSize: 18 }}>{p2Icon(st)}</span>
                  <span className="mono day-modal-total">{spunte} fatte · {saltate} saltate</span>
                </>; })()}
            </div>
            <div className="day-modal-list">
              {activeHabitsFor(habits, entry.date).map((h) => (
                <div className="day-modal-row" key={h.id}>
                  <span>{h.label}</span>
                  <span className={`mini-check ${entry.checks[h.id] ? "on" : "off"}`}>{entry.checks[h.id] ? "✔" : "✗"}</span>
                </div>
              ))}
            </div>
          </>
...
function Page1({ entries, settings, upsertEntry, deleteEntry, askConfirm, dark }) {
  ...
  function jumpToForm(d) {
    setDrillDay(null);
    if (editingId) { pendingDateRef.current = d; setEditingId(null); }
    else { setDate(d); }
  ...
  useEffect(() => {
    if (editingId) {
      ...
      if (pendingDateRef.current) { setDate(pendingDateRef.current); pendingDateRef.current = null; }
  ...
  const activeCats = useMemo(() => activeCategoriesFor(settings, date), [settings, date]);
  const previewTotal = activeCats.reduce((s, c) => s + (Number(minutes[c.key]) || 0), 0);
  const previewStatus = p1Status(previewTotal, settings);

  function submit() {
    if (!date) return;
    const fullMinutes = { ...minutes };
    activeCats.forEach((c) => { fullMinutes[c.key] = Number(minutes[c.key]) || 0; });
    ...
    <div>
      ...
      <div className="section">
        ...
        <MiniCalendar
          ...
          statusFn={(e) => p1Status(p1Total(e, settings), settings)}
          detailFn={(e) => `${p1Total(e, settings)} min`}
      ...
      <div className="section" ref={formRef}>
        ...
        {activeCats.length < settings.categories.length && (
          <div className="dim" style={{ fontSize: 11, marginTop: -6, marginBottom: 10 }}>Alcune categorie non sono ancora mostrate: sono state aggiunte dopo questa data.</div>
        )}
                ...
                return (
                  <tr key={e.id}>
                    ...
                    {settings.categories.map((c) => (
                      <td className="mono" key={c.key}>{isActiveOn(c.sinceDate, e.date) ? (e.minutes[c.key] || 0) : <span className="dim">—</span>}</td>
                    ))}
...
function Page2({ entries, habits, upsertEntry, deleteEntry, askConfirm }) {
  ...
  const activeHabits = useMemo(() => activeHabitsFor(habits, date), [habits, date]);
  function toggle(id) { setDraft((d) => ({ ...d, [id]: !d[id] })); }
  function markAll() { const d = {}; activeHabits.forEach((h) => { d[h.id] = true; }); setDraft(d); }

  let previewSpunte = 0; activeHabits.forEach((h) => { if (draft[h.id]) previewSpunte++; });
  const previewSaltate = activeHabits.length - previewSpunte;
  const previewStatus = activeHabits.length > 0 && previewSpunte === activeHabits.length ? "verde" : previewSaltate <= 1 ? "giallo" : "rosso";

  function submit() {
    if (!date) return;
    const checks = { ...draft };
    activeHabits.forEach((h) => { if (checks[h.id] === undefined) checks[h.id] = false; });
    let targetId = editingId;
    if (!targetId) {
      const dup = entries.find((e) => e.date === date);
      if (dup) targetId = dup.id;
  ...
  return (
    <div>
      ...
      <div className="section">
        ...
        <MiniCalendar
          cursor={cursor} setCursor={setCursor} entries={entries}
          statusFn={(e) => p2Status(e, habits)}
          detailFn={(e) => `${p2Counts(e, habits).spunte}/${habits.length}`}
      ...
      <div className="section" ref={formRef}>
        ...
        {activeHabits.length < habits.length && (
          <div className="dim" style={{ fontSize: 11, marginTop: 8 }}>Alcuni habit non sono ancora mostrati: sono stati aggiunti dopo questa data.</div>
        )}
        <div className="form-preview">
              ...
              {[...sorted].reverse().map((e) => {
                ...
                return (
                  <tr key={e.id}>
                    ...
                    <td className="mono">{fmtDateIt(e.date)}</td>
                    {habits.map((h) => (
                      <td key={h.id} className={isActiveOn(h.sinceDate, e.date) ? `mini-check ${e.checks[h.id] ? "on" : "off"}` : "dim"}>
                        {isActiveOn(h.sinceDate, e.date) ? (e.checks[h.id] ? "✔" : "✗") : "—"}
                      </td>
                    ))}
...
function SettingsDrawer({ open, onClose, settings, setSettings, habits, setHabits, saveSettings, saveHabits, askConfirm }) {
  ...
  function updateCategoryLabel(key, val) {
    setLocal((s) => ({ ...s, categories: s.categories.map((c) => (c.key === key ? { ...c, label: val } : c)) }));
  }
  function addCategory() {
    const v = newCategory.trim();
    if (!v) return;
    setLocal((s) => ({ ...s, categories: [...s.categories, { key: genId(), label: v, sinceDate: todayStr() }] }));
    setNewCategory("");
  }
  function removeCategory(key, label) {
    askConfirm({
      title: `Rimuovere "${label}"?`,
      message: "Non sarà più mostrata nei nuovi inserimenti. I dati già registrati per questa categoria restano salvati e visibili nel registro.",
      onConfirm: () => setLocal((s) => ({ ...s, categories: s.categories.filter((c) => c.key !== key) }))
    });
  }
  ...
  function addHabit() {
    ...
    if (!v) return;
    setLocalHabits((h) => [...h, { id: genId(), label: v, sinceDate: todayStr() }]);
    setNewHabit("");
  }
  function removeHabit(id, label) {
    askConfirm({
      title: `Rimuovere "${label}"?`,
      message: "Non sarà più mostrato nei nuovi inserimenti. I dati già registrati restano salvati e visibili nel registro.",
      onConfirm: () => setLocalHabits((h) => h.filter((x) => x.id !== id))
    });
  }
      ...
      <div className="drawer">
        ...
        <div className="drawer-section">
          ...
          {local.categories.map((c) => (
            <div className="habit-edit-row" key={c.key}>
              <input type="text" value={c.label} onChange={(e) => updateCategoryLabel(c.key, e.target.value)} />
              <button className="icon-btn danger" onClick={() => removeCategory(c.key, c.label)}><Icon.trash width={12} height={12} /></button>
            </div>
          ))}
          <div className="habit-edit-row" style={{ marginTop: 8 }}>
            <input type="text" placeholder="Nuova categoria..." value={newCategory} onChange={(e) => setNewCategory(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") addCategory(); }} />
            <button className="icon-btn" onClick={addCategory}><Icon.plus width={14} height={14} /></button>
          </div>
          <div className="dim" style={{ fontSize: 11, lineHeight: 1.5, marginTop: 8 }}>Una categoria aggiunta ora comparirà nei nuovi inserimenti a partire da oggi — i giorni passati la mostreranno come "—" invece di 0.</div>
        </div>
        ...
        <div className="drawer-section" style={{ borderBottom: "none" }}>
          ...
          {localHabits.map((h) => (
            <div className="habit-edit-row" key={h.id}>
              ...
              <button className="icon-btn danger" onClick={() => removeHabit(h.id, h.label)}><Icon.trash width={12} height={12} /></button>
            </div>
          ))}
          <div className="habit-edit-row" style={{ marginTop: 8 }}>
  ...
  return (
    <div id="app">
      ...
      <SettingsDrawer
        ...
        saveSettings={saveSettingsToDb} saveHabits={saveHabitsToDb} askConfirm={askConfirm}
      />
...
.cal-cell{aspect-ratio:1/0.95;border-radius:7px;border:1px solid var(--line-soft);background:var(--panel-alt);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:2px;position:relative;font-size:10.5px;color:var(--ink-dim);}
...
.cal-cell.clickable:hover{filter:brightness(1.06);transform:translateY(-1px);}
.cal-cell .detail{font-size:8px;font-weight:600;color:var(--ink-dim);}
.cal-cell.verde .detail,.cal-cell.giallo .detail,.cal-cell.rosso .detail{color:inherit;opacity:0.85;}
...
.day-modal{background:var(--panel);border:1px solid var(--line);border-radius:12px;padding:20px;max-width:360px;width:100%;}
.day-modal-head{display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;}
.day-modal-head h4{font-size:15px;}
.day-modal-empty{text-align:center;padding:10px 4px 4px;}
.day-modal-empty p{color:var(--ink-dim);font-size:12.5px;margin:0 0 14px;}
.day-modal-badge{display:flex;align-items:center;gap:10px;margin-bottom:14px;}
.day-modal-total{font-size:13px;font-weight:700;}
.day-modal-list{display:flex;flex-direction:column;gap:2px;}
.day-modal-row{display:flex;justify-content:space-between;align-items:center;padding:8px 2px;border-bottom:1px solid var(--line-soft);font-size:12.5px;}
