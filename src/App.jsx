...
function RidgeChart({ entries, settings }) {
  ...
  const colorFor = (s) => (s === "verde" ? "#5F8F6B" : s === "giallo" ? "#C1893A" : "#B0472F");
      ...
      <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`}>
        <defs>
          <linearGradient id="ridgeFill" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#5F8F6B" stopOpacity="0.28" />
            <stop offset="100%" stopColor="#5F8F6B" stopOpacity="0.02" />
          </linearGradient>
        </defs>
        <path d={areaPath} fill="url(#ridgeFill)" />
        <path d={linePath} fill="none" stroke="#2F4B3C" strokeWidth="1.6" />
        {pts.map((p, i) => (
          <circle key={i} cx={p.x} cy={p.y} r="4.2" fill={colorFor(p.status)} stroke="#F8F6EF" strokeWidth="1.4">
            <title>{fmtDateIt(p.date)} — {p.total} min</title>
          </circle>
        ))}
        {pts.map((p, i) => (i % showEvery === 0 ? (
          <text key={"t" + i} x={p.x} y={h - 8} fontSize="8.5" fill="#666C5D" textAnchor="middle" fontFamily="JetBrains Mono, monospace">{i + 1}</text>
        ) : null))}
      </svg>
    </div>
  );
...
function Page1({ entries, settings, upsertEntry, deleteEntry, askConfirm }) {
  ...
  return (
    <div>
      ...
      <div className="section">
        ...
        <RidgeChart entries={entries} settings={settings} />
...
export default function App() {
  ...
  return (
    <div id="app">
      ...
      {tab === "page1"
        ? <Page1 entries={minutiEntries} settings={settings} upsertEntry={upsertMinuti} deleteEntry={deleteMinuti} askConfirm={askConfirm} />
...
.hero-progress-label{font-size:12px;color:#CFE0D2;text-transform:uppercase;letter-spacing:0.07em;font-weight:600;}
...
.hero-fill{height:100%;background:linear-gradient(90deg,var(--moss),#8FCF9E);border-radius:6px;transition:width .35s ease;}
...
.cal-cell.skipped{background:repeating-linear-gradient(135deg, rgba(176,71,47,0.16) 0 6px, rgba(176,71,47,0.28) 6px 12px);border-color:var(--rust);color:var(--rust-deep);}
.cal-cell.verde{background:rgba(95,143,107,0.18);border-color:var(--moss);color:var(--moss-deep);}
.cal-cell.giallo{background:rgba(193,137,58,0.18);border-color:var(--amber);color:var(--amber-deep);}
.cal-cell.rosso{background:rgba(176,71,47,0.18);border-color:var(--rust);color:var(--rust-deep);}
...
.chip-toggle.on{border-color:var(--moss);background:rgba(95,143,107,0.1);}
...
.badge.verde{background:rgba(95,143,107,0.16);color:var(--moss-deep);}
.badge.giallo{background:rgba(193,137,58,0.16);color:var(--amber-deep);}
.badge.rosso{background:rgba(176,71,47,0.16);color:var(--rust-deep);}
...
.backdrop{position:fixed;inset:0;background:rgba(35,42,34,0.45);z-index:60;display:flex;justify-content:flex-end;}
...
.toast.success{background:#E4EFE5;border:1px solid var(--moss);color:var(--moss-deep);}
.toast.error{background:#F5E4E0;border:1px solid var(--rust);color:var(--rust-deep);}
